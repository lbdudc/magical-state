<template>
  <v-row align="center" no-gutters class="mb-1">
    <v-col cols="auto">
      <v-btn class="first-button" :disabled="disablePlayButton"  elevation="0" @click="play">
        <v-icon class="icon">mdi-play</v-icon>
      </v-btn>
      <v-btn class="middle-button" :disabled="disableStopButton" elevation="0" @click="stop">
        <v-icon class="icon">mdi-stop</v-icon>
      </v-btn>
    </v-col>
    <v-col class="speed-selector-container">
      <v-select
        :class="hasInstantSelectorFunction ? 'middle-selector' : 'last-selector'"
        color="#c3c9d0"
        elevation="0"
        dense
        :label="label"
