...
describe("Store", () => {
  ...
  it("should throw error if store is not correct", () => {
    ...
    ).rejects.toThrow("jsonSpec is not defined");
  });
  ...
  it("should create a correct observable", async () => {
    ...
    obs.forEach((el, index) => {
      ...
      expect(el.setDefaultFirstItem).toBe(
        jsonSpec[index].setDefaultFirstItem || false
      );
      expect(el.setItemsOnMounted).toBe(
        jsonSpec[index].setItemsOnMounted || false
      );
    });
  });
  ...
  it("should return the expected selector", async () => {
    ...
    expect(selector.setDefaultFirstItem).toEqual(
      jsonSpec[index].setDefaultFirstItem || false
    );
    expect(selector.setItemsOnMounted).toEqual(
      jsonSpec[index].setItemsOnMounted || false
    );
  ...
  it("should return expected items from getValues", async () => {
    ...
    const store = await createStore(
      simpleJsonSpec,
      () => new Promise((resolve) => resolve(mockItems)),
      null,
      () => {}
    );
  ...
  it("should throw error callback is not a function on store creation", () => {
    expect(async () =>
      createStore(simpleJsonSpec, getValues, null, null)
    ).rejects.toThrow("callback is not a function");
  });
  ...
  it("should reject setSelector if desired value not found in selector items", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => {});
    expect(store.setSelector("DATE_FILTER", 50)).rejects.toEqual(
      "The value 50 is not in the selector items, selector with DATE_FILTER not set"
    );
  });

  it("should reject change if new value is an array and the selector is not multiple", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => {});
    expect(store.change("DATE_FILTER", [1])).rejects.toEqual(
      "Cannot set value of type array on a selector that is not multiple"
    );
  });

  it("should reject setState if new value is an array and the selector is not multiple", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => {});
    expect(store.setSelector("DATE_FILTER", [1, 2])).rejects.toEqual(
      "Cannot set value of type array on a selector that is not multiple"
    );
  });
});
