...
describe("Store", () => {
  ...
  it("should reject setSelector if desired value not found in selector items", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => { });
    expect(store.setSelector("DATE_FILTER", 50)).
      rejects.toEqual("The value 50 is not in the selector items, selector with DATE_FILTER not set");
  });

  it("should reject change if new value is an array and the selector is not multiple", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => { });
    expect(store.change("DATE_FILTER", [1])).
  ...
  it("should reject setState if new value is an array and the selector is not multiple", async () => {
    const store = await createStore(simpleJsonSpec, getValues, null, () => { });
    expect(store.setSelector("DATE_FILTER", [1, 2])).
      rejects.toEqual("Cannot set value of type array on a selector that is not multiple");
  })
