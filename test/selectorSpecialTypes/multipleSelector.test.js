...
describe("multipleSelector", () => {
  ...
  it("should reject setSelector if desired value not found in selector items", async () => {
    ...
    expect(store.setSelector("SPATIAL_AGGREGATION", [50])).rejects.toEqual(
      "The value 50 is not in the selector items, selector with SPATIAL_AGGREGATION not set"
    );
  });
});
